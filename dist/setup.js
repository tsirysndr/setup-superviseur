...
export default async (options) => {
    ...
    if (cacheEnabled) {
        ...
        if (cacheRestored) {
            ...
            if (version) {
                ...
                action.warning("Found a cached version of Superviseur, but it appears to be corrupted? Attempting to download a new version.");
            }
        }
    }
    if (!cacheHit) {
    ...
    if (cacheEnabled) {
        try {
            ...
            action.warning(`Failed to save the downloaded version of Superviseur to the cache: ${error.message}`);
        }
    }
    return {
        version,
